...
export default function ZikrCalculatorPage() {
  ...
  const calculate = () => {
    ...
    if (errs.length) { setErrors(errs); setResults(null); return; }
    setErrors([]);
    setResults(PRESET_COUNTS.map(({ count, label }) => ({ count, label, seconds: count * secs })));
  };
    ...
    <main className="ow-page min-h-screen">
      ...
      <div className="relative z-10 max-w-xl mx-auto px-4 py-10 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <h1 className="text-ow-cream text-xl font-bold">📿 حاسبة الذكر</h1>
          <div className="w-16" />
        </div>
        <ZikrForm
          zikrText={zikrText}
